...
**Wave is pre-computed:** Wave numbers are assigned during `/gwd:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/get-work-done/templates/user-setup.md` for full schema and examples
...
See `~/.claude/get-work-done/references/goal-backward.md` for derivation process.
See `~/.claude/get-work-done/workflows/verify-phase.md` for verification logic.
