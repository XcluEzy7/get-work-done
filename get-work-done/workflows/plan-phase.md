...
1. ~/.claude/get-work-done/templates/phase-prompt.md
2. ~/.claude/get-work-done/references/plan-format.md
3. ~/.claude/get-work-done/references/scope-estimation.md
4. ~/.claude/get-work-done/references/checkpoints.md
5. ~/.claude/get-work-done/references/tdd.md
...
For niche domains (3D, games, audio, shaders, ML), suggest `/gwd:research-phase` before plan-phase.
</step>
...
- Any {phase}-RESEARCH.md (from /gwd:research-phase)
- Any DISCOVERY.md (from mandatory discovery)
- Any {phase}-CONTEXT.md (from /gwd:discuss-phase)
...
# Check for ecosystem research (from /gwd:research-phase)
cat .planning/phases/XX-name/${PHASE}-RESEARCH.md 2>/dev/null

# Check for phase context (from /gwd:discuss-phase)
...
**If neither exist:** Suggest /gwd:research-phase for niche domains, /gwd:discuss-phase for simpler domains, or proceed with roadmap only.
</step>
...
See `~/.claude/get-work-done/references/tdd.md` for TDD plan structure.
...
See ~/.claude/get-work-done/references/checkpoints.md for checkpoint structure.
...
See ~/.claude/get-work-done/references/scope-estimation.md for complete guidance.
</step>
...
Use template from `~/.claude/get-work-done/templates/phase-prompt.md`.
...
**Wave is pre-computed:** Wave numbers are assigned during planning (see `assign_waves` step). `/gwd:execute-phase` reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
```markdown
<execution_context>
@~/.claude/get-work-done/workflows/execute-plan.md
@~/.claude/get-work-done/templates/summary.md
@~/.claude/get-work-done/references/checkpoints.md
</execution_context>
```
...
- `/gwd:execute-plan {phase}-01-PLAN.md` - run plans one at a time
...
**TDD candidates get dedicated plans.** If "Create price calculator with discount rules" warrants TDD, create a TDD plan for it. See `~/.claude/get-work-done/references/tdd.md` for TDD plan structure.
</task_quality>
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/gwd:execute-phase {X}` next
</success_criteria>
