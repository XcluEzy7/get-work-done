...
**No dependency analysis needed.** Wave numbers are pre-computed during `/gwd:plan-phase`.
...
2. **Spawn all autonomous agents in wave simultaneously:**
   ...
   @~/.claude/get-work-done/references/tdd.md
   </execution_context>
...
| `gaps_found` | Present gap summary, offer `/gwd:plan-phase {phase} --gaps` |
...
- `/gwd:verify-work {X}` — manual testing before planning
```

User runs `/gwd:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) to close gaps
3. User then runs `/gwd:execute-phase {X}` again
...
1. Run `/gwd:execute-phase {phase}` again
