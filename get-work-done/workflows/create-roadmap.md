...
1. ~/.claude/get-work-done/templates/roadmap.md
2. ~/.claude/get-work-done/templates/state.md
3. ~/.claude/get-work-done/templates/requirements.md
...
- NOT for scope creep or "nice to haves" (capture with /gwd:add-todo instead)
...
Use template from `~/.claude/get-work-done/templates/roadmap.md`.

Initial roadmaps use integer phases (1, 2, 3...).
Decimal phases added later via /gwd:insert-phase command (if it exists).
...
**If NEW_STATE:** Create fresh using template from `~/.claude/get-work-done/templates/state.md`.
