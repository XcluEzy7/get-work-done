...
See ~/.claude/get-work-done/references/checkpoints.md for complete checkpoint guidance.
</step>
...
If you're reading this workflow because an orchestrator spawned you (vs running directly from /gwd:execute-plan), the orchestrator's prompt will include checkpoint return instructions. Follow those instructions when you hit a checkpoint.
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/get-work-done/templates/user-setup.md`.
...
Use ~/.claude/get-work-done/templates/summary.md for structure.
...
For commit message conventions, see ~/.claude/get-work-done/references/git-integration.md
</step>
...
`/gwd:execute-plan .planning/phases/{phase-dir}/{phase}-{next-plan}-PLAN.md`
...
**Also available:**
- `/gwd:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/gwd:verify-work {Z}` — manual acceptance testing before continuing
...
**Also available:**
- `/gwd:verify-work` — manual acceptance testing before completing milestone
- `/gwd:add-phase <description>` — add another phase before completing
