...
Triggered by /gwd:research-phase command when the domain is niche, complex, or Claude's training is likely stale.
...
Use /gwd:progress to see available phases.
```
Exit workflow.
...
**2. Phase context (if exists from /gwd:discuss-phase):**
```bash
...
**From ~/.claude/get-work-done/references/research-pitfalls.md:**
...
Use template from ~/.claude/get-work-done/templates/research.md
...
What's next?
1. Plan this phase (/gwd:plan-phase ${PHASE}) - RESEARCH.md will be loaded automatically
...
When /gwd:plan-phase runs after research:
