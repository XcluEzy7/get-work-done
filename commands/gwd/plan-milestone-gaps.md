...
<objective>
Create all phases necessary to close gaps identified by `/gwd:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/gwd:add-phase` per gap.
</objective>
...
```
No audit gaps found. Run `/gwd:audit-milestone` first.
```
...
**Also available:**
- `/gwd:execute-phase {N}` — if plans already exist
...
`/gwd:audit-milestone` — re-audit to verify gaps closed
`/gwd:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/gwd:plan-phase` next
</success_criteria>
