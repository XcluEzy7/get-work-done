...
@~/.claude/get-work-done/references/questioning.md
@~/.claude/get-work-done/templates/project.md
@~/.claude/get-work-done/templates/config.json
...
1. **Abort if project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] && echo "ERROR: Project already initialized. Use /gwd:progress" && exit 1
   ```
...
- options:
  - "Map codebase first" — Run /gwd:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**
```
Run `/gwd:map-codebase` first, then return to `/gwd:new-project`
```
Exit command.
...
Present completion with next steps (see ~/.claude/get-work-done/references/continuation-format.md):
